...
[^组件绑定事件备注]:  组件绑定事件的的时候,会调用$on来绑定相应的事件,它会判断如果是绑定多事件,遍历event数组,使用$on来绑定相应的事件及回调函数

```
如果要在组件上使用原生事件，需要加.native修饰符，这样就相当于在父组件中把子组件当做普通html标签，然后加上原生事件。
...
[具体链接]: https://blog.csdn.net/weixin_43390711/article/details/106677793
